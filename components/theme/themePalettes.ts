/**
 * Named color presets — flat ThemeColors snapshots (same shape as Copy config palettes).
 * Defaults re-exported from the kit; named presets live only here / on the docs site.
 */
import type { ThemeColors } from "burne-ui/internal";
import { DARK_COLORS, LIGHT_COLORS } from "burne-ui/internal";

export { DARK_COLORS, LIGHT_COLORS };

export const AMBER_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#1a1610",
  secondary: "#2a241c",
  secondaryForeground: "#f4f5f7",
  tertiary: "#332c24",
  tertiaryForeground: "#f4f5f7",
  border: "#4a3d2a",
  foreground: "#f4f5f7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#a89478",
  primary: "#d97706",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#b45309",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const AMBER_LIGHT_COLORS: ThemeColors = {
  background: "#fafafa",
  surface: "#ffffff",
  secondary: "#ebebec",
  secondaryForeground: "#18181b",
  tertiary: "#e2e2e4",
  tertiaryForeground: "#18181b",
  border: "#f8e0c4",
  foreground: "#18181b",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#71717a",
  primary: "#d97706",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#b45309",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const AUTUMN_DARK_COLORS: ThemeColors = {
  background: "#01141e",
  surface: "#0b1b28",
  secondary: "#12283b",
  secondaryForeground: "#f9f6e7",
  tertiary: "#1a3d5c",
  tertiaryForeground: "#f9f6e7",
  border: "#1a4a6e",
  foreground: "#f9f6e7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#a8c4d4",
  primary: "#d62828",
  primaryForeground: "#f4f8fb",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#d62828",
  indicatorForeground: "#f4f8fb",
  danger: "#d62828",
  success: "#51d280",
  info: "#4cc9f0",
  warning: "#fcbf49",
  primaryTint: "color-mix(in oklab, var(--color-primary) 14%, var(--color-surface))",
  primaryTintStrong: "color-mix(in oklab, #fcbf49 22%, var(--color-surface))",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const AUTUMN_LIGHT_COLORS: ThemeColors = {
  background: "#f4f8fb",
  surface: "#ffffff",
  secondary: "#e8f0f6",
  secondaryForeground: "#01141e",
  tertiary: "#dce8f0",
  tertiaryForeground: "#01141e",
  border: "#b8d4e8",
  foreground: "#01141e",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#5c7a8a",
  primary: "#d62828",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#d62828",
  indicatorForeground: "#ffffff",
  danger: "#d62828",
  success: "#49d07a",
  info: "#0891b2",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 14%, var(--color-surface))",
  primaryTintStrong: "color-mix(in oklab, #fcbf49 22%, var(--color-surface))",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const BERRY_DARK_COLORS: ThemeColors = {
  background: "#0c0306",
  surface: "#240f16",
  secondary: "#371a22",
  tertiaryForeground: "#f5ece6",
  secondaryForeground: "#f5ece6",
  tertiary: "#4b202d",
  border: "#37252b",
  foreground: "#f5ece6",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#c97a88",
  primary: "#db4852",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#890620",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const BERRY_LIGHT_COLORS: ThemeColors = {
  background: "#f1e4df",
  surface: "#f5ece6",
  secondary: "#e5dad4",
  secondaryForeground: "#2c0703",
  tertiary: "#ddd2cc",
  tertiaryForeground: "#2c0703",
  border: "#da9f93",
  foreground: "#2c0703",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#7a3040",
  primary: "#b6465f",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#890620",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const BOLD_DARK_COLORS: ThemeColors = {
  background: "#0c0a14",
  surface: "#131221",
  secondary: "#1a1834",
  secondaryForeground: "#f0f4ff",
  tertiary: "#242042",
  tertiaryForeground: "#f0f4ff",
  border: "#332b45",
  foreground: "#f0f4ff",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#8899cc",
  primary: "#f72585",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#f72585",
  indicatorForeground: "#ffffff",
  danger: "#f72585",
  success: "#4cc9f0",
  info: "#4361ee",
  warning: "#f77f00",
  primaryTint: "color-mix(in oklab, var(--color-primary) 16%, var(--color-surface))",
  primaryTintStrong: "color-mix(in oklab, #4361ee 24%, var(--color-surface))",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const BOLD_LIGHT_COLORS: ThemeColors = {
  background: "#f8f6ff",
  surface: "#ffffff",
  secondary: "#ede8ff",
  secondaryForeground: "#1e1b4b",
  tertiary: "#e4dcff",
  tertiaryForeground: "#1e1b4b",
  border: "#d4c4ff",
  foreground: "#1e1b4b",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#6366a8",
  primary: "#d61f6d",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#d61f6d",
  indicatorForeground: "#ffffff",
  danger: "#d61f6d",
  success: "#0891b2",
  info: "#4361ee",
  warning: "#ea580c",
  primaryTint: "color-mix(in oklab, var(--color-primary) 16%, var(--color-surface))",
  primaryTintStrong: "color-mix(in oklab, #4361ee 24%, var(--color-surface))",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const CHERRY_DARK_COLORS: ThemeColors = {
  background: "#0c0404",
  surface: "#281010",
  secondary: "#3a1818",
  secondaryForeground: "#fff0ee",
  tertiary: "#4e1d1d",
  tertiaryForeground: "#fff0ee",
  border: "#3e2828",
  foreground: "#fff0ee",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#c08080",
  primary: "#ad2831",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#800e13",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const CHERRY_LIGHT_COLORS: ThemeColors = {
  background: "#f5e8e8",
  surface: "#fff0ee",
  secondary: "#eededd",
  secondaryForeground: "#250902",
  tertiary: "#e6d6d5",
  tertiaryForeground: "#250902",
  border: "#e8bfb8",
  foreground: "#250902",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#8a4540",
  primary: "#ad2831",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#800e13",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const CONTRAST_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#171717",
  secondary: "#282a2e",
  secondaryForeground: "#ffffff",
  tertiary: "#313438",
  tertiaryForeground: "#ffffff",
  border: "#3d4250",
  foreground: "#ffffff",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#a8adb8",
  primary: "#ffffff",
  primaryForeground: "#0c0c0e",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#ffffff",
  indicatorForeground: "#0c0c0e",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const CONTRAST_LIGHT_COLORS: ThemeColors = {
  background: "#ffffff",
  surface: "#f4f5f7",
  secondary: "#e8eaed",
  secondaryForeground: "#0c0c0e",
  tertiary: "#dce0e6",
  tertiaryForeground: "#0c0c0e",
  border: "#c8ccd4",
  foreground: "#0c0c0e",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#5c6370",
  primary: "#0c0c0e",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#0c0c0e",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const DREAMLAND_DARK_COLORS: ThemeColors = {
  background: "#1a1428",
  surface: "#241e34",
  secondary: "#2e2840",
  secondaryForeground: "#f5e8fc",
  tertiary: "#382e4c",
  tertiaryForeground: "#f5e8fc",
  border: "#5a4870",
  foreground: "#ffe8f3",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#b89ac8",
  primary: "#ffafcc",
  primaryForeground: "#1a1428",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#cdb4db",
  indicatorForeground: "#1a1428",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const DREAMLAND_LIGHT_COLORS: ThemeColors = {
  background: "#fef8fc",
  surface: "#ffffff",
  secondary: "#ffe8f3",
  secondaryForeground: "#3d2d50",
  tertiary: "#e8f4fe",
  tertiaryForeground: "#3d2d50",
  border: "#cdb4db",
  foreground: "#3d2d50",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#9a7aab",
  primary: "#ff70a5",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#7eb0f0",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const EARTHY_DARK_COLORS: ThemeColors = {
  background: "#12100c",
  surface: "#1a1610",
  secondary: "#242018",
  secondaryForeground: "#f8f4e8",
  tertiary: "#2e2820",
  tertiaryForeground: "#f8f4e8",
  border: "#4a4030",
  foreground: "#f8f4e8",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#9a8878",
  primary: "#adc178",
  primaryForeground: "#0d0d0d",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#6c584c",
  indicatorForeground: "#adc178",
  danger: "#d73737",
  success: "#adc178",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const EARTHY_LIGHT_COLORS: ThemeColors = {
  background: "#faf9f5",
  surface: "#f8f4e8",
  secondary: "#ece6d4",
  secondaryForeground: "#3d3228",
  tertiary: "#e4ddd0",
  tertiaryForeground: "#3d3228",
  border: "#a98467",
  foreground: "#3d3228",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#6c584c",
  primary: "#adc178",
  primaryForeground: "#0d0d0d",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#6c584c",
  indicatorForeground: "#adc178",
  danger: "#df4e4e",
  success: "#adc178",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const EMERALD_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#121716",
  secondary: "#1f332e",
  secondaryForeground: "#f4f5f7",
  tertiary: "#273d38",
  tertiaryForeground: "#f4f5f7",
  border: "color-mix(in oklab, var(--color-foreground) 12%, transparent)",
  foreground: "#f4f5f7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#8b90a0",
  primary: "#34d399",
  primaryForeground: "#f4f5f7",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#10b981",
  indicatorForeground: "#f4f5f7",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const EMERALD_LIGHT_COLORS: ThemeColors = {
  background: "#fafffb",
  surface: "#ffffff",
  secondary: "#dcfce7",
  secondaryForeground: "#064e3b",
  tertiary: "#d1fae5",
  tertiaryForeground: "#064e3b",
  border: "#bbf7d0",
  foreground: "#01130e",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#6b7280",
  primary: "#059669",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#10b981",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const HARVEST_DARK_COLORS: ThemeColors = {
  background: "#190c06",
  surface: "#20150e",
  secondary: "#2f2014",
  secondaryForeground: "#ffe6a7",
  tertiary: "#462c1b",
  tertiaryForeground: "#ffe6a7",
  border: "#362a21",
  foreground: "#ffe6a7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#aea089",
  primary: "#bb9457",
  primaryForeground: "#432818",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#bb9457",
  indicatorForeground: "#2f2714",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const HARVEST_LIGHT_COLORS: ThemeColors = {
  background: "#fffcf5",
  surface: "#fffef5",
  secondary: "#fce8c0",
  secondaryForeground: "#432818",
  tertiary: "#f5dcc8",
  tertiaryForeground: "#432818",
  border: "#dad7d2",
  foreground: "#432818",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#99582a",
  primary: "#6f1d1b",
  primaryForeground: "#fff8e8",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#bb9457",
  indicatorForeground: "#432818",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const LAVENDER_DARK_COLORS: ThemeColors = {
  background: "#16162a",
  surface: "#22223b",
  secondary: "#2c2e48",
  secondaryForeground: "#f2e9e4",
  tertiary: "#363852",
  tertiaryForeground: "#f2e9e4",
  border: "#4a4e69",
  foreground: "#f2e9e4",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#9a8c98",
  primary: "#c9ada7",
  primaryForeground: "#22223b",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#9a8c98",
  indicatorForeground: "#22223b",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const LAVENDER_LIGHT_COLORS: ThemeColors = {
  background: "#f2e9e4",
  surface: "#faf7f5",
  secondary: "#ebe3de",
  secondaryForeground: "#22223b",
  tertiary: "#e5ddd8",
  tertiaryForeground: "#22223b",
  border: "#c9ada7",
  foreground: "#22223b",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#9a8c98",
  primary: "#4a4e69",
  primaryForeground: "#f2e9e4",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#9a8c98",
  indicatorForeground: "#f2e9e4",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const MYSTIC_DARK_COLORS: ThemeColors = {
  background: "#111014",
  surface: "#1a161d",
  secondary: "#2b2136",
  secondaryForeground: "#eacdc2",
  tertiary: "#4a355c",
  tertiaryForeground: "#eacdc2",
  border: "#3a2942",
  foreground: "#eacdc2",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#999495",
  primary: "#b75d69",
  primaryForeground: "#fafafa",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#b75d69",
  indicatorForeground: "#eacdc2",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const MYSTIC_LIGHT_COLORS: ThemeColors = {
  background: "#faf5f3",
  surface: "#fff9f7",
  secondary: "#f5ebe6",
  secondaryForeground: "#1a1423",
  tertiary: "#eacdc2",
  tertiaryForeground: "#1a1423",
  border: "#c9a8b0",
  foreground: "#1a1423",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#774c60",
  primary: "#774c60",
  primaryForeground: "#fff9f7",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#372549",
  indicatorForeground: "#fff9f7",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const OCEAN_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#16191d",
  secondary: "#202a37",
  secondaryForeground: "#f4f5f7",
  tertiary: "#283442",
  tertiaryForeground: "#f4f5f7",
  border: "color-mix(in oklab, var(--color-foreground) 12%, transparent)",
  foreground: "#f4f5f7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#8b90a0",
  primary: "#38bdf8",
  primaryForeground: "#000000",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#06b6d4",
  indicatorForeground: "#000000",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const OCEAN_LIGHT_COLORS: ThemeColors = {
  background: "#f0f9ff",
  surface: "#ffffff",
  secondary: "#e0f2fe",
  secondaryForeground: "#0c4a6e",
  tertiary: "#dbeafe",
  tertiaryForeground: "#0c4a6e",
  border: "#bae6fd",
  foreground: "#03131c",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#64748b",
  primary: "#0284c7",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#06b6d4",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const PAPRIKA_DARK_COLORS: ThemeColors = {
  background: "#120806",
  surface: "#1c1008",
  secondary: "#281810",
  secondaryForeground: "#ede8e2",
  tertiary: "#322018",
  tertiaryForeground: "#ede8e2",
  border: "#5c3828",
  foreground: "#ede8e2",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#c49a88",
  primary: "#e16036",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#e3170a",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const PAPRIKA_LIGHT_COLORS: ThemeColors = {
  background: "#f5f0ea",
  surface: "#ede8e2",
  secondary: "#ddd6d0",
  secondaryForeground: "#200d06",
  tertiary: "#d5cec8",
  tertiaryForeground: "#200d06",
  border: "#dcb5a7",
  foreground: "#200d06",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#9a6a5e",
  primary: "#e16036",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#e3170a",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const PEACH_DARK_COLORS: ThemeColors = {
  background: "#141010",
  surface: "#1c1614",
  secondary: "#282018",
  secondaryForeground: "#fff5ee",
  tertiary: "#322820",
  tertiaryForeground: "#fff5ee",
  border: "#5c4038",
  foreground: "#fff5ee",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#c99a9a",
  primary: "#f08080",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#e06b6b",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const PEACH_LIGHT_COLORS: ThemeColors = {
  background: "#ffffff",
  surface: "#fff5ee",
  secondary: "#fce8dc",
  secondaryForeground: "#5c2d2d",
  tertiary: "#f8e0d4",
  tertiaryForeground: "#5c2d2d",
  border: "#f8ad9d",
  foreground: "#5c2d2d",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#c97a7a",
  primary: "#f08080",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#e06b6b",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const ROSE_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#1a1216",
  secondary: "#2a2228",
  secondaryForeground: "#f4f5f7",
  tertiary: "#332a32",
  tertiaryForeground: "#f4f5f7",
  border: "#4a3540",
  foreground: "#f4f5f7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#a88a94",
  primary: "#f43f5e",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#e11d48",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const ROSE_LIGHT_COLORS: ThemeColors = {
  background: "#fafafa",
  surface: "#ffffff",
  secondary: "#ebebec",
  secondaryForeground: "#18181b",
  tertiary: "#e2e2e4",
  tertiaryForeground: "#18181b",
  border: "#f5e6e7",
  foreground: "#18181b",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#71717a",
  primary: "#f43f5e",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#e11d48",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const RUSTIC_DARK_COLORS: ThemeColors = {
  background: "#141210",
  surface: "#1c1a18",
  secondary: "#262220",
  secondaryForeground: "#fffcf2",
  tertiary: "#302c28",
  tertiaryForeground: "#fffcf2",
  border: "#4a4038",
  foreground: "#fffcf2",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#a89888",
  primary: "#eb5e28",
  primaryForeground: "#fffcf2",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#c94a1e",
  indicatorForeground: "#fffcf2",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const RUSTIC_LIGHT_COLORS: ThemeColors = {
  background: "#f6f3ef",
  surface: "#fffcf2",
  secondary: "#f5f0e8",
  secondaryForeground: "#252422",
  tertiary: "#ede8df",
  tertiaryForeground: "#252422",
  border: "#8a837a",
  foreground: "#252422",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#403d39",
  primary: "#eb5e28",
  primaryForeground: "#fffcf2",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#c94a1e",
  indicatorForeground: "#fffcf2",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const SAND_DARK_COLORS: ThemeColors = {
  background: "#141210",
  surface: "#1c1814",
  secondary: "#262018",
  secondaryForeground: "#f5ebe0",
  tertiary: "#302820",
  tertiaryForeground: "#f5ebe0",
  border: "#4a4038",
  foreground: "#f5ebe0",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#a89888",
  primary: "#9a846f",
  primaryForeground: "#f5ebe0",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#7a6a58",
  indicatorForeground: "#f5ebe0",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const SAND_LIGHT_COLORS: ThemeColors = {
  background: "#f5ebe0",
  surface: "#f4e4d2",
  secondary: "#ebe3d8",
  secondaryForeground: "#3a342f",
  tertiary: "#e3d9ce",
  tertiaryForeground: "#3a342f",
  border: "#d5bdaf",
  foreground: "#3a342f",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#8a7f76",
  primary: "#9a846f",
  primaryForeground: "#f5ebe0",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#7a6a58",
  indicatorForeground: "#f5ebe0",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const SLATE_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#14161f",
  secondary: "#22242e",
  secondaryForeground: "#f4f5f7",
  tertiary: "#2a2d3a",
  tertiaryForeground: "#f4f5f7",
  border: "#3d4250",
  foreground: "#f4f5f7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#8b90a0",
  primary: "#6366f1",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#6366f1",
  indicatorForeground: "#ffffff",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const SLATE_LIGHT_COLORS: ThemeColors = {
  background: "#fafafa",
  surface: "#ffffff",
  secondary: "#ebebec",
  secondaryForeground: "#18181b",
  tertiary: "#e2e2e4",
  tertiaryForeground: "#18181b",
  border: "#e2e8f0",
  foreground: "#18181b",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#71717a",
  primary: "#6366f1",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#6366f1",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const TOFFEE_DARK_COLORS: ThemeColors = {
  background: "#130d06",
  surface: "#241a10",
  secondary: "#382a1f",
  secondaryForeground: "#f8f3ee",
  tertiary: "#543a27",
  tertiaryForeground: "#f8f3ee",
  border: "#5c4030",
  foreground: "#f8f3ee",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#b08968",
  primary: "#9c6644",
  primaryForeground: "#f8f3ee",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#7f5539",
  indicatorForeground: "#f8f3ee",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const TOFFEE_LIGHT_COLORS: ThemeColors = {
  background: "#ede0d4",
  surface: "#f8f3ee",
  secondary: "#e8e1dc",
  secondaryForeground: "#2d1208",
  tertiary: "#e0d9d4",
  tertiaryForeground: "#2d1208",
  border: "#b08968",
  foreground: "#2d1208",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#9c6644",
  primary: "#7f5539",
  primaryForeground: "#f8f3ee",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#9c6644",
  indicatorForeground: "#f8f3ee",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#d97706",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const VIOLET_DARK_COLORS: ThemeColors = {
  background: "#0c0d10",
  surface: "#17161d",
  secondary: "#28243a",
  secondaryForeground: "#f4f5f7",
  tertiary: "#312d45",
  tertiaryForeground: "#f4f5f7",
  border: "color-mix(in oklab, var(--color-foreground) 12%, transparent)",
  foreground: "#f4f5f7",
  muted: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-surface))",
  mutedForeground: "#8b90a0",
  primary: "#a354f2",
  primaryForeground: "#f4f5f7",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#8b5cf6",
  indicatorForeground: "#f4f5f7",
  danger: "#d73737",
  success: "#51d280",
  info: "#57b1db",
  warning: "#d8a44b",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#fafafa",
  successForeground: "#fafafa",
  infoForeground: "#fafafa",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 38%, transparent)",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};

export const VIOLET_LIGHT_COLORS: ThemeColors = {
  background: "#faf5ff",
  surface: "#ffffff",
  secondary: "#ede9fe",
  secondaryForeground: "#3b0764",
  tertiary: "#e4d9fc",
  tertiaryForeground: "#3b0764",
  border: "#ddd6fe",
  foreground: "#0e0317",
  muted: "color-mix(in oklab, var(--color-foreground) 12%, var(--color-surface))",
  mutedForeground: "#6b7280",
  primary: "#7c3aed",
  primaryForeground: "#ffffff",
  focusRing: "color-mix(in oklab, var(--color-primary) 60%, transparent)",
  focusRingDanger: "color-mix(in oklab, var(--color-danger) 60%, transparent)",
  focusRingSuccess: "color-mix(in oklab, var(--color-success) 60%, transparent)",
  focusRingInfo: "color-mix(in oklab, var(--color-info) 60%, transparent)",
  focusRingWarning: "color-mix(in oklab, var(--color-warning) 60%, transparent)",
  indicator: "#8b5cf6",
  indicatorForeground: "#ffffff",
  danger: "#df4e4e",
  success: "#49d07a",
  info: "#50b0dc",
  warning: "#dda646",
  primaryTint: "color-mix(in oklab, var(--color-primary) 20%, transparent)",
  primaryTintStrong: "color-mix(in oklab, var(--color-primary) 25%, transparent)",
  dangerForeground: "#ffffff",
  successForeground: "#ffffff",
  infoForeground: "#ffffff",
  warningForeground: "#0c0c0e",
  primaryHover: "color-mix(in oklab, var(--color-primary) 80%, var(--color-surface))",
  defaultHover: "color-mix(in oklab, var(--color-foreground) 8%, var(--color-surface))",
  transparentHover: "color-mix(in oklab, var(--color-muted) 70%, transparent)",
  secondaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-secondary))",
  tertiaryHover: "color-mix(in oklab, var(--color-foreground) 6%, var(--color-tertiary))",
  surfaceTintDanger: "color-mix(in oklab, var(--color-danger) 15%, transparent)",
  surfaceTintDangerHover: "color-mix(in oklab, var(--color-danger) 25%, transparent)",
  dangerFillHover: "color-mix(in oklab, var(--color-danger) 80%, transparent)",
  surfaceTintSuccess: "color-mix(in oklab, var(--color-success) 15%, transparent)",
  surfaceTintSuccessHover: "color-mix(in oklab, var(--color-success) 25%, transparent)",
  successFillHover: "color-mix(in oklab, var(--color-success) 80%, transparent)",
  surfaceTintInfo: "color-mix(in oklab, var(--color-info) 15%, transparent)",
  surfaceTintInfoHover: "color-mix(in oklab, var(--color-info) 25%, transparent)",
  infoFillHover: "color-mix(in oklab, var(--color-info) 80%, transparent)",
  surfaceTintWarning: "color-mix(in oklab, var(--color-warning) 15%, transparent)",
  surfaceTintWarningHover: "color-mix(in oklab, var(--color-warning) 25%, transparent)",
  warningFillHover: "color-mix(in oklab, var(--color-warning) 80%, transparent)",
  convergeRipplePrimaryFill: "color-mix(in oklab, var(--color-primary-foreground) 52%, var(--color-primary))",
  convergeRippleNeutral: "var(--color-primary-tint-strong)",
  convergeRippleNeutralMuted: "color-mix(in oklab, var(--color-primary) 14%, transparent)",
  convergeRippleDanger: "color-mix(in oklab, var(--color-danger-foreground) 38%, transparent)",
  convergeRippleSuccess: "color-mix(in oklab, var(--color-success-foreground) 38%, transparent)",
  convergeRippleInfo: "color-mix(in oklab, var(--color-info-foreground) 38%, transparent)",
  convergeRippleWarning: "color-mix(in oklab, var(--color-warning-foreground) 38%, transparent)",
};
