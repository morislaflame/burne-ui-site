...
`frontHeight` (ResizeObserver → ref Map, без React state) + `extraPeek` → сразу GSAP `to(container, { height })` / `style.height` при reduced motion. Viewport не ре-рендерится от RO.
